    ...
    Student.findOne({ "id": id }, function (err, data) {
      if (err)
        ...
        if (data.BalanceOfPayment > 0) { return resolve(1); }
          ...
          // Object.entries(data.tests).forEach(([key, value]) => {
          //   if (value == "") {
          //     d = 1;
  ...
  try {
    ...
    if (student_1) {
      ...
      myDoc.info = {
        ...
        Subject: `אישור לימודים לשנת תשפ"ב`,
        Keywords: "אישור לימודים",
        CreationDate: new Date(),
        ModDate: new Date(),
        Creator: "STUSENT CERTIFICATE",
        Producer: "pdfkit"
      };
      const text = ` ${recipientType} אנו מאשרים כי`;
      const text2 = ` ${student_1.firstName}  ${student_1.lastName}` + ` מ.ז. ${student_1.id}`;
      const text3 = `לומדת במוסדנו "מכון בית יעקב למורות ירושלים" בשנת הלימודים תשפ''ב  `;
