  ...
  return (
    <div style={{ marginTop: '15vh', backgroundColor: '#ffffffbd', padding: '20px' }}>
      ...
      <Table striped bordered hover size="sm" style={{ backgroundColor: 'ffffff8a' }}>
        ...
        <tbody>
          {arrStudents ? arrStudents.sort(((a, b) => a.lastName > b.lastName ? 1 : -1)).map((item, key) => (
            <tr key={key + item.phone}>
              <td>{arrUpdate[key] ? <input defaultValue={item.id} onChange={(e) => setProp(key, 'id', e.target.value)}></input> : item.id}</td>
              ...
              <td>{arrUpdate[key] ? <input defaultValue={item.phone} onChange={(e) => setProp(key, 'phone', e.target.value)}></input> : item.phone}</td>
              ...
              <td>{arrUpdate[key] ? <input style={{ width: '60px' }} defaultValue={item.BalanceOfPayment} onChange={(e) => setProp(key, 'BalanceOfPayment', e.target.value)}></input> : item.BalanceOfPayment}</td>
              <td>{arrMarks[key] ?
      ...
      {/* {isAddMark ?
        <Modal.Dialog>
          <Modal.Header>
            <Modal.Title>הוספת ציון</Modal.Title>
          </Modal.Header>
          ...
          <Modal.Body >
            ...
            <Autocomplete disablePortal
              ...
              options={arrSubject.map(x => x.subject).concat('אחר')}
              sx={{ width: 223 }}
              renderInput={(params) => <TextField {...params} label='בחרי מקצוע' />} />
            {subject == 'אחר' ? <TextField required onChange={(e) => setNewSubject(e.target.value)} type="text" label='הכנס מקצוע חדש' /> : null}
            <br /><TextField required onChange={(e) => setMark(e.target.value)} type="text" label='הכנס ציון'></TextField>
          </Modal.Body>

          <Modal.Footer>
            <Button variant="secondary" onClick={() => setIsAddMark(false)}>ביטול</Button>
            <Button variant="primary" onClick={() => addTest()}>שמור</Button>
          </Modal.Footer>
        </Modal.Dialog> : null} */}
